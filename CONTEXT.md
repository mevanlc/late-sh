...
- Last updated: 2026-09-16 (local development uses thin `Makefile` / `Makefile2` wrappers over `common.mk`, selecting the committed `.env.dev` / `.env.dev2` templates and matching compiled profiles; see §10.1)
- Status: Active
...
Compose project, network, and volumes. Both Makefiles include `common.mk` and
copy `.env.dev` or `.env.dev2` verbatim to `.env`. Edit the committed templates
for Compose settings and the matching `dev` / `dev2` profiles in each app's
`config.rs` for application settings. Check database defaults live in the
Makefile wrappers. `make start-instance2` also selects `Makefile2`.
