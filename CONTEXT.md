...
- `Hub` (in `app/hub`) is the Shop modal, a single Shop tab titled " Shop ". Ctrl+S, the `/shop` composer command (`open_shop_modal_globally`), and the locked pet/aquarium nudges open it; profile/job editors and active Artboard input retain their local Ctrl+S bindings. The hub module owns the leaderboard/quest/shop services even though leaderboards render on page `6` and quests render on The Arcade. Hub-owned marketplace and entitlement projection code lives under `app/hub/shop`. Detailed behavior lives in `late-ssh/src/app/hub/CONTEXT.md`.
...
| `Ctrl+S` / `/shop` | Global chord / chat composer | Open the Shop modal. Profile/directory editors keep save, job forms keep post, and active Artboard input keeps slot 2. The locked pet/aquarium nudges open the same modal programmatically |
