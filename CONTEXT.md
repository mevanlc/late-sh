...
- **Defaults follow the upstream bar.** `default_enabled()` is pomodoro/voice/mentions/pot/chips in upstream order. The pot is enabled but low-priority, compacts from `pot 84,200 · 3h12m` to `pot 84,200`, and drops before an established readout yields. A component added to the roster later backfills at its own `backfill_existing()` (the pot is the sole forced-on addition), not at a blanket `true` like the sidebar's rule.
