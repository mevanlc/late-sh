...
| `Shift+↑` / `Shift+↓` / `Space` / `→` | Settings modal bottom status bar customizer (Tweaks > Bottom status bar) | Move the selected segment along the bar (`[`/`]` alias), turn it on/off, or step into its options; Keyhints offers a Brief on/off option. `Esc` backs out of the options, then closes |
