...
- **Ideas do not live here.** Roadmap items, "future work", parked designs, research notes, and "we could" belong in a design doc (`GAME.md`, `DIGEST.md`, `SCALE.md`). A context file records what exists and how it behaves; an open gap is recorded as a fact ("X does not do Y"), and the proposal to close it goes to a design doc.
...
- **Cross-session fan-out goes through `LISTEN/NOTIFY`,** not through a process-local `broadcast`. There is one LISTEN connection per process (`late-ssh/src/pg_listener.rs`): add the channel to its closed `Channel` enum, `subscribe` your domain's channels in `main.rs`, and run a `start_notify_worker` over the queue. The worker gets `Signal::Resync` once the LISTEN is live (re-read here: it is the seed and the reconnect catch-up) and `Signal::Notify { channel, payload }` per notify; it re-reads into a process-shared `watch` that sessions read. Never open a domain-owned LISTEN connection. Shapes to copy: news and flags (re-read only, a burst collapses into one read), crown and pot (payload applied in order), shop (several channels, a failed notify reconciles).
...
**Replica-ready, not over-engineered.** The rule above is about correctness: no wrong writes, no double payouts, no switch that flips on one replica only. It is not a promise that every screen is live everywhere. These are acceptable by default and need no extra machinery:

- **One user, two sessions on different replicas** (laptop and phone): an action in one does not have to reach the other live. It shows up on the other's next reload or reconnect. Do not add a notify or a listener just for this case.
- **Stale until the page is opened again**: a list or badge that refreshes when the user enters the page, opens the modal, or logs in is replica-ready. Work and Showcase badges (read at login, cleared on visit) and the Profiles page list (re-read on entry) are the worked examples.
- **Live across replicas only where people sit and watch**: a surface users keep open while others change it (News, chat, the crown, the pot) gets a `LISTEN/NOTIFY`-fed shared `watch`. Everything else reads from the DB when it is opened.

When choosing, pick the cheapest shape that is correct: read on open, then a notify, and only then anything more. A design that adds a listener, a channel, or a per-user event to cover one of the acceptable cases above is over-engineered.
...
- It crosses replicas because stickiness is per connection, not per user: one account's laptop and phone can sit on different replicas. Bonsai gets that live sync for free from a notify it already needs for its own writes; a new domain does not need to build it for this case alone (see "Replica-ready, not over-engineered" above). It is sent unconditionally because knowing whether another session exists would take a cross-replica presence registry, which the rule forbids.
...
| `late-ssh/src/app/deadchannel/CONTEXT.md` | The deadchannel game domain (design docs `GAME.md` and `DIGEST.md` live in that directory): the first-contact haunting ladder (clock glitch, name flicker, splash whisper, invitation DM), the eligibility gate (tenure, touched settings, AI-screened bio), the glyph alphabet, `/haunt` admin controls, the haunt kill switch and fuse (`app_flags`), the runner and its look (`deadchannel_runners`, the piece table, the #deadchannel portrait gutter), the night city under the clubhouse, or the fight (the runner's sheet and day roll on the row, the ration fight at the screen, the armorer's till, the wire's news lines), or the tailor (the mirror that edits the look). | Domain layout (game root + `haunt/` onboarding subdomain + `runner/` character layer + `city/` the street + `fight/` the sheet and the fight + `tailor/` the look's editor), the four stage machines and their chain (hits arm the whisper, the whisper schedules the invitation), the gate and its thresholds, the replica rule as applied (row claims for every cap and stamp, flags through `app/flags`, looks through the `deadchannel_runner_changed` directory), persisted `first_contact_*` marks, the runner row and the look JSON contract, the `afterglow` ghost voice, render hooks, `/haunt` command surface, tests, and gotchas. |
...
| `late-ssh/src/app/paper/CONTEXT.md` | The Late Edition: `/paper`, the once-a-day login pop, the sweeper that prints one column per public room, the Elsewhere / What we were reading / Outside pages, the NEW WORK section, the `paper_*` app flags, or the `paper_at_login` tweak. | Press versus newsstand split, the row-claim print contract (`printing` / `ready` / `quiet`, stale reclaim), which rooms qualify, the layout order (rail order, favorites first), the login claim, telemetry, tests, and gotchas. |
| `late-ssh/src/app/jobs/CONTEXT.md` | The job feed: the Jobs shelf on the Profiles page, `/jobs`, the post form (`n`, `/jobs post`, `d` takes yours down), the nightly press (HN Who is hiring, We Work Remotely, Jobicy), the tag vocabulary, the FOR YOU matches under a work card, the paper's NEW WORK rows, the `jobs_enabled` flag, or the `job_postings` / `job_press_runs` tables. | The once-a-night press under a daily run claim, the 14-day HN drip, the model read and its schema, the per-replica shelf snapshot, tag matching, telemetry, tests, and gotchas. |
...
A blanket `changed = true` or fixed cadence needs a written justification at its call site (current survivors, all in tick.rs: splash typing, lobby modal 1Hz occupancy, DailyMatch 1Hz deadline clock, the `anim_half` ~7.5fps edge shared by the pet box, bonsai sway, and clubhouse ambience, and the `anim_quarter` ~3.8fps edge for aquarium steps). The program summary, design rules, test gotchas, and open follow-ups live in SCALE.md (Render-Cost Program).
...
- **HTTP client spans:** `TracedExt::send_traced` names the span `{method} {scheme://host/path}` and records the same in `url.full`; the query string and fragment never reach a span (`telemetry.rs::span_url`), because traces are readable by anyone with Grafana and Google APIs accept keys as `?key=`. Send API keys in headers anyway (Gemini uses `x-goog-api-key`).
...
- **Chat username badge order:** Chat author labels render top-3 last-completed-UTC-month leaderboard award badges first as one bracketed group, then special allowlist badges (`mod`, `developer`, `artist` order), the bonsai glyph, rented chat badge, rented flag, burn milestone, then the LIVE tag and finally the away glyph `💤` (no word) once every active session for that user is away: 30 quiet minutes, `/brb`, or IRC `AWAY` (`late-ssh/src/app/common/away.rs`, in memory on the `active_users` roster, resolved once a second). The bonsai glyph is the precomputed `bonsai_trees.badge_glyph` (ladder `·` `⚘` `🌱` `🌲` `🌳` `🌸` `🌼`, scored from live branch presence times health); a dead tree, or an account that has not logged in since the reset, shows none.
...
| **Profiles** | 5 | Active | The work page: two shelves, People and Jobs, `Space` between them. People is one row per user who posted a work card or shared a project, sorted by latest activity; each row is three fixed lines (name + status dot + age, headline or newest project, five tags + project count). The detail pane sits beside the list from 100 cols (stacked under it: `l`/`Enter` opens it over the list, `h`/`Esc` returns) and shows the person in sections: header (`@user · status · type · location`), CARD, ABOUT (bio), PROJECTS, then late.fetch as a dim block last. `h`/`l` move a focus cursor over the card and projects; `Enter` copies the focused link, `e` edits it, `d` deletes it (own, or any for a moderator). `w` opens the profile editor, `i` opens it on a blank project, `o` opens the read-only profile modal, `s` searches, `/` filters to your row. Jobs holds the remote job feed (`late-ssh/src/app/jobs/CONTEXT.md`). Lives in `late-ssh/src/app/directory/`. |
| _Profile editor_ | — | Active | One modal (`late-ssh/src/app/directory/editor/`) for everything a person shows on page 5, three pages switched with `Tab`: `card` (headline, status, type, location, contact, links, skills, summary), `about` (bio, ide, terminal, os, langs: the same rows Settings has, saved through the same profile call), `projects` (the list, `a` adds, `Enter` edits, `d` deletes; each project is a small form). Rows are `label  value`; `j`/`k` move, `Enter` types into a row (Enter/Tab commit and step to the next), `←`/`→` cycle status and type, `Ctrl+S` saves every dirty page and closes (on a project form it saves that project and returns to the list), `Esc` stops typing, then asks before discarding. Status and type are closed enums (`late_core::models::work_profile`); skills and langs are picked, not typed: `Enter` on either row opens the tag picker (`late-ssh/src/app/tag_picker/`, also the langs row of Settings), a filtered multi-select over the tag vocabulary (`late-core/src/vocab.rs`; langs offer its language group alone), Space/Enter pick, Backspace drops the last pick, Esc hands the list back; `skills` and `skills_tags` hold the same canonical tags, twelve at most. A card typed before the picker opens with what the vocabulary knows and nothing else. A moderator editing someone else's card or project gets that page alone. |
...
Framed pages use the same component renderer on both horizontal borders; Zen stays frameless and clears the status-bar click targets. The top-right bar is fixed UI policy — voice, mentions, the Deadchannel runner readout outside the city, pot, chips — and shares its row with the page tabs. The bottom-left bar is user-arranged and shares its row with the optional sponsor title. Its persisted model lives in `late-core/src/models/statusline.rs` (`StatusComponent` roster — Keyhints, voice, mentions, pot, chips, your move, invites, quests, station, users online, time — plus `LabelMode`, `StatusVariant`, and the `parse_/normalize_/*_json` trio); rendering and hit-testing live in `late-ssh/src/app/statusline/`. The bottom arrangement is stored account-wide in `users.settings.statusline_components`; per-device scoping is not designed yet.
...
- **Top and bottom policy are independent.** `build_top_status_bar()` reproduces the upstream HUD, fitting the runner readout before the pot and is not persisted. The bottom bar defaults to the Keyhints component alone; every status reading is opt-in and starts low-priority. The full Keyhints component retains its dotted, spaced, and caret-control compactions and may be disabled to free the row. Its Brief property defaults off and paints `⚙ ^o · ⚄ ^g · ◉ ^s` with no further compaction when enabled. It is stored as `brief` on the `shortcuts` entry; saved selections of the former brief component fold into Keyhints, preferring full hints if both were enabled. A component added to the roster later backfills at its own `backfill_existing()`; Keyhints is forced on and inserted at the front when missing, while optional status components append disabled.
- Customizer: Settings > Statusline, a top-level tab immediately after Tweaks. The selected component has a concise, wrapped description above its options. `late-ssh/src/app/settings_modal/` — `StatuslinePane`/`StatuslineDial` in `state.rs`, `draw_statusline_tab` in `ui.rs`. `Tab`/`Shift+Tab` switch tabs from either pane; `Enter` opens the options, `Esc` returns to the list, and `Left`/`Right` change the focused option.
...
- Outer frame: the default bottom-left Keyhints component (`StatusComponent::Shortcuts`) advertises `Settings Ctrl+O`, `Lobby Ctrl+G`, `Zen Ctrl+F`, `Shop Ctrl+S`, `Guide ?`, and `Exit qq`; users may disable it in the bottom status bar customizer.
...
| `Ctrl+R` | Reserved global, except active Artboard editing and an open chat composer (there it is redo, see `late-ssh/src/app/chat/CONTEXT.md`; `/redraw` repaints from inside one) | Force a full repaint: clears the client screen and re-emits every cell via `App::force_full_repaint`. The escape hatch when something outside late.sh scribbles on the terminal. |
...
| `Ctrl+S` / `/shop` | Global chord / chat composer | Open the Shop modal. Active games keep their controls; profile/directory editors keep save, job forms keep post, and active Artboard input keeps slot 2. The locked pet/aquarium nudges open the same modal programmatically |
...
| `Space` | Profiles (page 5) | Switch between the People and Jobs shelves |
| `w` / `i` / `e` | Profiles (page 5) | Open the profile editor: on your card, on a blank project, or on the focused item |
| `Tab` / `Shift+Tab`, `j`/`k`, `Enter`, `←`/`→`, `Ctrl+S`, `Esc` | Profile editor | Switch page; move rows; type into a row; cycle status or type; save; stop typing, then close (asks before discarding) |
...
2a. **Guide-owned Pair/Terminal/Economy topics** — `app/help_modal/data.rs`, `app/help_modal/terminal_faq.rs`, and `app/help_modal/hub_guide.rs` when changing pairing, OSC 52 / mouse / notification accuracy claims, chip/economy facts, or per-terminal click/select modifiers; also the bottom-left hint copy in `app/statusline/bar.rs::shortcut_spans()`
