...
| `late-ssh/src/app/hub/CONTEXT.md` | The `/shop` Shop modal, the Leaderboards page data/service, the Arcade quest strip's service, marketplace, pet/aquarium unlocks, or chip economy presentation. | Shop modal ownership, leaderboard rosters/refresh, reward/economy rules, daily/weekly quest service, marketplace and entitlement projection, aquarium tray behavior, and known gaps. |
...
The system is a Rust workspace with four main crates (`late-cli`, `late-core`, `late-ssh`, `late-web`) — plus `late-webview` (the CLI's embedded YouTube helper, split out so `late` never links WebKitGTK on Linux) and the standalone door hosts (`late-brogue`, `late-codekeep`, `late-dcss`, `late-dopewars`, `late-nethack`, `late-usurper`) — backed by PostgreSQL, Icecast audio streaming, Liquidsoap playlist management, and LiveKit voice media.
...
- `make check` is the full pre-merge sweep: `cargo fmt --check` for first-party packages, `cargo clippy` over the whole workspace with `--features otel`, and `cargo nextest` over the whole workspace. It is the only gate that compiles the otel telemetry/metrics code, since CI skips otel to stay cheap; otel breakage surfaces here or at the release Docker build, never in prod.
- `make check` starts a dedicated checkout-scoped Compose Postgres project from `docker-compose.check.yml` (`late-sh-check-<instance>-<worktree-scope>`, `CHECK_PG_HOST_PORT ?= 55433`) and tears it down with volumes. `CHECK_INSTANCE` changes the suffix but cannot remove the `late-sh-check-` ownership prefix. Checks must not start, stop, or reuse the app `postgres` service from `docker-compose.yml`.
...
- `LeaderboardService` exposes a shared `watch::Receiver<Arc<LeaderboardData>>` refreshed from DB every 5 minutes, and only while at least one session is subscribed (`REFRESH_INTERVAL` + `has_subscribers` in `app/hub/svc.rs`; a refresh with no listeners is nine aggregate queries published to nobody). Two rules stop that cadence from showing as an empty screen: `App::new` **seeds** from the already-published snapshot with `borrow()` (`watch::Sender::subscribe` marks the current value seen, so the `has_changed()` gate in `app/tick.rs` never fires for it), and `subscribe` wakes the loop so a session connecting to a snapshot already older than `REFRESH_INTERVAL` buys exactly one refresh — the age bound is what keeps a connect storm off the hot path. The data model is roster-generated (`late-core/src/models/leaderboard.rs`): the `DailyPuzzle` and `ScoreGame` closed enums drive every derived surface, so a new game added to a roster automatically joins its per-game monthly/all-time board, Arcade Wins points, today's champions, and the daily completion statuses; the per-board queries union all roster arms and rank with `PARTITION BY game`, which is how the full board set stays at nine queries. Contains today's champions, daily completion statuses, per-puzzle daily-win boards and per-score-game high-score boards (both `BoardWindows { monthly, all_time }`), monthly net chip-delta earners excluding shop purchases/floor restores, monthly Arcade champion points, and every user's chip balance. The Leaderboards page (screen `6`, `late-ssh/src/app/leaderboard/`) renders it as a board list + detail view with an around-you tail; Arcade Wins weighting comes from `Difficulty::points` (easy/draw-1 = 1, medium = 3, hard/draw-3 = 5), the same enum whose `chips()` carries the daily-win payout tiers. A companion hourly task idempotently snapshots top-3 previous-UTC-month leaderboard placements into permanent `profile_awards`; profile overview shows up to six earned awards plus `+N more` when present and always appends a compact `Badge Codes` legend after Showcases at the end of the scrollable overview. Chat author labels show top-3 last-completed-UTC-month award badges as one bracketed group. Top Chips badges render as `CHIP1`/`CHIP2`/`CHIP3`. Lateania final-boss achievements also write one-time `profile_awards` (`LAD`, `LFK`) and chat author labels include the best Lateania badge regardless of award month, collapsing `LAD` when `LFK` is present while profile views still show both awards. The NetHack door writes the same kind of one-time `profile_awards` for its two milestones (`NHA` acquiring the Amulet of Yendor, `NHY` ascending), shown regardless of award month and collapsing `NHA` when `NHY` is present.
...
Deploy workflows (`deploy.yml`, `deploy_web.yml`, `deploy_infra.yml`, `deploy_cli.yml`) trigger on published GitHub Releases and also support manual `workflow_dispatch` with explicit `release_tag` and `environment` inputs. Shared `ci`, `build`, and `terraform` workflow calls accept `source_ref`, so manual deploys check out the requested tag instead of the UI-selected branch. Use this as the recovery path when GitHub misses a release event. Door-game deploy workflows (`deploy_nethack.yml`, `deploy_dopewars.yml`, `deploy_dcss.yml`, `deploy_usurper.yml`, `deploy_brogue.yml`, `deploy_codekeep.yml`) are image-only: `ci` + `build`, then `kubectl set image` on that game's deployment (init and main container) and a rollout wait. They never run terraform on ordinary releases, so nothing else in the cluster is touched; the Dockerfile gives each door its own `builder-<door>` stage, so a door image build compiles only that host crate, never `late-ssh`/`late-web`. Each also self-bootstraps identically: a `check_deployment` job reads live cluster state, and when the game's deployment is missing (first deploy, disaster recovery) the run switches to `terraform.yml` with a `targets` input scoping the apply to exactly that game's resources plus dependencies; no manual dispatch needed. Other manifest changes ship through `deploy_infra.yml`, whose `get_images` job reads every live image tag off the cluster and therefore requires all game deployments to already exist.
...
**Key enums:**
- `Screen`: `Dashboard`, `Arcade`, `Games`, `Artboard`, `Lateania`, `Rebels`, `Nethack`, `Dopewars`, `Codekeep`, `GreenDragon`, `Pinstar`, `Clubhouse`, `DailyMatch`, `HouseTable` (the door-game screens — Lateania/Rebels/Nethack/Dopewars/Codekeep/GreenDragon — are reached only through the Games hub, and `DailyMatch`/`HouseTable` only from the Lobby modal; none are in the tab cycle. `Pinstar` renders as Directory (Profiles, Projects, and Pinstar tabs) on key `5`; `Clubhouse` is the tavern on key `0`, first in the Tab cycle and the landing screen for every session — see `late-ssh/src/app/clubhouse/`). `Dashboard` is rendered as Home and owns the chat room rail/center. News, Mentions, RSS, Voice, and Discover are synthetic room-like entries within Home chat. Showcase/Projects and Work/Profiles data still use chat-adjacent services and unread cursors, but their UI lives on Directory page 5, not the Home rail or room jump picker.
...
| **Leaderboards** | 6 | Active | Board list + detail page: Top Chips and Arcade Wins, then per-game daily-win boards (`DailyPuzzle` roster) and high-score boards (`ScoreGame` roster), each with monthly and all-time columns and an around-you tail. `j`/`k` or arrows select a board. Lives in `late-ssh/src/app/leaderboard/`. |
| _Lateania / NetHack / DCSS / Brogue / Usurper / Green Dragon / A Dark Room / Rebels / dopewars / CodeKeep_ | — | Active | Live door-game screens, not top-level tabs. Launched only from the Games hub (page 3); `Esc` (Lateania) or quitting the game (Rebels/NetHack/DCSS/Brogue/Usurper/dopewars/CodeKeep, e.g. `Ctrl-C`, `Q` at Usurper's menus, or `S` save in the roguelikes) returns to the hub. Per-game behavior lives in each door's CONTEXT.md (`lateania/`, `greendragon/`, `darkroom/`, `nethack/`, `dcss/`, `brogue/`, `usurper/`, `dopewars/`, `codekeep/`). |
