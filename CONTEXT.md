...
- Outer frame: the default bottom-left Keyhints component (`StatusComponent::Shortcuts`) advertises `Settings Ctrl+O`, `Lobby Ctrl+G`, `Zen Ctrl+F`, `Shop /shop`, `Guide ?`, and `Exit qq`; users may disable it in the bottom status bar customizer.
...
| `/shop` | Chat composer | Open the Shop modal. The locked pet/aquarium nudges open the same modal programmatically |
