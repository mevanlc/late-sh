...
| `ui.rs` | Ratatui rendering for class select, log, compact mode, the live field (full-width on narrow terminals), side panels, hints. The Character panel expands to a full-width dashboard (accent-tinted class portrait, dot-rated ability scores, vitals/XP meters) when the area is at least 72x18, else falls back to the narrow side panel. **That dashboard has no scroll of its own** (`[`/`]` only reach `draw_side`), so every column has to fit: keep unbounded lists (titles) last and capped, and Experience above them. Foes/Adventurers/Follow render as aligned roster rows with HP meters. Lock-free, snapshot-only. |
...
| `world.rs` | Immutable world data and generation: rooms, exits, mobs, features, wildlife, overworld, Frontier. Also **resource nodes** (`NODES`/`ResourceNode`/`nodes_at`/`node_index`): trees, ore veins, fishing spots and herb/skinning patches keyed to rooms, modelled exactly like `WILDLIFE` (static data + a per-node service cooldown), each with a skill, tier, min-level gate, and derived yield item. |
...
UI uses a two-column layout. The compact fallback (`draw_compact`) takes terminals narrower than 50 columns or shorter than 9 rows, except that with RPG mode on the Room panel checks for the narrow field layout first (any width under 96 with 8+ rows, see below), so there compact applies only under 8 rows. The left column splits current room context (`Now`) from newest-first action scrollback (`Recent`) with a visible divider; **while a fight is on** (a `targeted` mob, or a duel via a `targeted` occupant) the `Now` block is replaced by the **battle frame** (`ui::battle_context`): the foe's full name, its nature line (rank · attack school · weakness/resist, from the new `MobView.school/weak/resist`), wide HP meters for both sides, `afflicted:` (DoT stacks/stun) and the player's `shield/empowered/stunned` effects (new `PlayerView` fields), reverting the moment the fight ends. Note the battle frame lives in the *classic* layout's left column only, and with RPG mode on the Room panel never draws that layout (every width 8+ rows tall gets a field layout, see below): on 96+ columns the fight reads from the side panel's foe roster (the targeted foe's expanded traits line, see the Room panel note in §5); the narrow field layout has no side panel, so there it reads from `narrow_status_line` alone (your HP and resource, the locked foe's name and HP) with no nature line, afflictions, or player effects. RPG mode off keeps the classic layout and the battle frame at every width from 50 columns up. The `Now` region wraps the room description naturally and only truncates the whole context as a last resort to preserve recent-event space. Service room-description lines use `LogKind::Room` and are filtered out of `Recent` so movement does not bury combat, loot, chat, and system events. Arrivals use compact `LogKind::Travel` breadcrumbs so Recent still shows where the player has just been. Consecutive identical recent events are collapsed with an `xN` suffix so repeated blocked-movement warnings do not flood the split.
**The map is never in the side rail.** With RPG mode on, the Room panel is the live field at every width: 96+ columns get field + room rail + full-width log strip; narrower (a phone) gets `ui::draw_narrow_field` - a one-row status bar (`narrow_status_line`: HP, resource, locked foe, room name), the field across the full width in the centre, and a small event feed under it (`narrow_log_height`, `height / 4` clamped 3..7). No rail there; the bottom action bar still carries the fight. The classic (RPG mode off) layout draws no map in its rail either; `m` opens the overhead map.
...
- `Dir` supports cardinal and vertical movement. `Dir::delta_2d` returns `None` for up/down because the overhead map grid is flat.
...
- `CraftEntryView` carries what the output **is**, not just its name (rarity, stats, desc, slot, `compare`/`compare_pct`/`worn_*`, the ingredient lines itemised as `CraftIngredientView { name, need, have }`, and `held`: the pack count for consumable/utility outputs, `None` for gear and crafted materials), because a craft is a gear decision the way a purchase is and the screen stands it against what is worn with the same `ItemDetail` the shop uses. The itemised ingredients are what let the screen name the material you are short of; `craftable`/`reason` alone could only say "need materials" and send a maker back to the shops guessing.

### Crafting depth [VOLATILE]

- **Weapon coats (five schools, one curve)**: using a crafted poison vial (`items::poison_tier`) or one of the four **weapon oils** (`items::oil_id`/`oil_school_tier`/`OIL_SCHOOLS` = Fire/Frost/Holy/Lightning, 6 tiers each, Alchemy recipes with a school-flavored second ingredient) routes out of the normal consumable path in `use_item` - via the single `items::coat_school_tier` lookup - and coats the weapon: `PlayerState::weapon_coat = Some((school, per_tick, charges))` (transient, one slot: any new coat replaces the last). Each landed melee strike seeds a DoT of the coat's school via `seed_mob_dot`, which bakes the foe's resist/weak multiplier into the per-tick up front, so a matched coat is a real matchup lever, and spends a charge. Coats seed pvp dots in duels the same way, and the active coat shows in both battle panels' `you:` effects line via `PlayerView.coat` (`CoatView { school, charges }`). Coats are always flat riders added to the Physical auto, never a conversion of its school and never a multiplier on `attack()` - that line is reserved for the planned Thundersmith class (THUNDERSMITH.md).
  - **One curve for every coat, so the school is the whole decision** (`COAT_PER_TICK` `[2,6,10,15,21,30]`, `COAT_CHARGES = 40`, `COAT_PRICE`, and the same two-herbs-plus-one-material recipe shape). Poison and oil were two curves - the vial bursting ~30% of the auto bar for 5 strikes, the oil sustaining ~20% for 12 - but that split was an accident of order rather than a design: `POISON_CHARGES` shipped with crafting in #425 and `OIL_CHARGES` arrived ~120 PRs later with the resist/weak pass, so the vial simply kept the older item's constants and the burst/sustain story was written around them afterwards. The seam was still visible in the names (`COAT_DOT_TICKS` was `POISON_DOT_TICKS` and governed oils too). Merged: what a coat *is* is now its school, and the world's resist/weak board prices that already. Poison keeps its own item ids (`poison_id`, not `oil_id(4, t)`) because those sit in saved inventories; `COAT_SCHOOLS` is the five-school set. Pinned by `every_coat_in_the_game_carries_the_same_rider_and_differs_only_by_school` and `the_coat_curve_stays_inside_its_share_of_the_bar`.
  - **`C` coats in one keystroke** (`svc::coat_best`, the sibling of `Q`/`quaff_best`), plus a `⚗` action-bar chip (`ClickAction::Coat`) that reads `x{charges}` while a coat is live. Because every coat carries the same rider, a "use the biggest vial" key would throw the matchup system away, so the pick is ordered: a school the current foe is **weak** to, then any it does not **resist**, then the highest tier, then `PlayerState::last_coat` (what you chose yourself). With no target locked on - the common case, at a zone gate - nothing is known about the matchup and it falls through to tier and habit. It refuses to overwrite a live coat of any school while more than `COAT_TOPUP_AT` charges remain, unless the locked-on foe makes the switch worth it (weak to the pick, or resisting what the weapon carries), so a better vial in the bag never costs you strikes on its own; a nearly spent coat still tops up or switches freely. Shift-`c` no longer opens the character sheet; plain `c` still does.
  - **One wound per coat, refreshed** (`svc::DotSource`): a coat re-seeds on every landed strike, at the very cadence its DoT ticks, so it keeps a single stack per attacker and refreshes it in place. Ability DoTs still stack, because a cooldown rations how often they can be cast. This is load-bearing, not housekeeping: while coats pushed a stack per strike, `COAT_DOT_TICKS` of them were live at once and every coat was silently worth three times its rider (`a_coat_keeps_one_refreshing_wound_however_many_swings_land`). The wound's source is persisted (`SavedMobDot::from_coat`) so a reload cannot untag a live coat and let a second stack open beside it. Only the opening of a wound is logged, never a refresh.
  - **Priced against a measured bar**: `svc::TIER_ATTACK_BAR` is a *measured* auto-attack yardstick (a real character at each crafting gate wearing that tier's crafted weapon, pinned by `the_attack_bar_still_matches_a_real_character`), and the coat curve is written as a share of it - ~20% of the auto, held for a whole zone run. `the_coat_curve_stays_inside_its_share_of_the_bar` holds every tier to that band, which is what stops the curve quietly outgrowing the attack curve (it had: the oil rider was documented at 15% of output while really running three to six times that).
...
- **Both halves of the matchup are on screen.** A foe's nature line names its attack school and its weak/resist; the ability rows name theirs, via `Ability::effect_label` ("fire damage over time", "arcane stun"), in the `v` panel and in the battle side panel's roster alike - both render the one `AbilityView.effect` built in `svc`, so there is a single place to change it. The school comes from `Ability::school`, which returns `None` for `Ward`/`Empower`/`Heal`/`HealOverTime`: `damage_type` is a plain field on every row of the roster, but only `Strike`/`DamageOverTime`/`Stun`/`Finisher` reach `damage_target`, so the rest carry an inert school that must never be shown as though it were a lever. `only_the_effects_that_resolve_damage_carry_a_school` pins that split against the runtime's own dispatch.
...
**The martial lever** is the weapon-coat family (the poison vial plus the four
Alchemy oils, five schools on one curve; mechanics in Crafting depth): flat,
charge-limited school riders on the Physical auto that ride `seed_mob_dot`'s
baked-in resist/weak multiplier.
...
`COAT_PER_TICK` over `TIER_ATTACK_BAR` (both in `svc.rs`, both pinned to a live
...
`the_coat_curve_stays_inside_its_share_of_the_bar`. Re-theming a zone means
...
`use_item` -> `coat_school_tier` -> `coat_weapon` path checks class. `weapon_coat`
is a single `Option`, so a new coat overwrites the old (wasting its charges):
**one school at a time, never two**. `DotSource::Coat` refreshes one wound rather
than stacking, so while charges last the coat is a flat `per_tick` every tick,
plus `COAT_DOT_TICKS` lingering ticks after the last swing.

**Coats out-cover the casters on the resist/weak board.** `COAT_SCHOOLS` is
Fire/Frost/Holy/Lightning/Poison. Against the 16 `ZoneTheme` weaknesses:

| kit | weak lanes covered |
|---|---|
| any class carrying all five coat schools | **12 / 16** |
...
- visited rooms, for the overhead map's fog of war;
...
- `world.rs`: exit validity, reachability, room count, overworld count, room description length, mob home validity, mob ID uniqueness, loot references, boss quest mapping, capital features, wildlife, early Frontier regular difficulty.
