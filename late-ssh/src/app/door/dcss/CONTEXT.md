...
- Last updated: 2026-08-05 (per-account init.txt paste box, plus a same-day revert: a defensive `-extra-opt-last macro_dir=` re-force briefly broke every DCSS launch in prod because `macro_dir` is a `DisabledGameOption` on this build; see §9's `macro_dir`/`save_dir` invariant)
- Status: Active
...
- While Running, raw client bytes are forwarded straight to the host→child (minus mouse/paste noise), so crawl — not late.sh — interprets keys. late.sh keeps two keys: `F1`, remapped to crawl's own `?` help (same rationale and encodings as nethack's remap), and `` ` ``, which **detaches**: the game keeps running (state + SSH connection survive `set_screen`) while the backtick workspace cycle hops to the next live dungeon or back to chat; resume from the hub card or the cycle. This deliberately costs crawl its own ` repeat-previous-command binding (players can macro another key to it in-game). A running game with no forwarded keystroke for 20 minutes is closed by the client (`IDLE_SHUTDOWN` in `state.rs`), which lands as the host's SIGHUP-save; an off-screen non-running state is reaped in `app/tick.rs`.
...
- CI: `.github/workflows/deploy_dcss.yml` builds and rolls out dcss, and only dcss, for `-dcss` releases: `ci` + `build`, then `kubectl set image` on the `late-dcss` deployment (both the `dcss-save-seed` init container and the main container) and a rollout wait. No terraform runs on ordinary releases, so nothing else in the cluster is touched. When the `late-dcss` deployment is missing the run auto-bootstraps (a terraform apply `-target`-scoped to the dcss resources only); other manifest changes go through `deploy_infra.yml`; other deploy workflows that do run terraform read the live tag off the `late-dcss` deployment and pass it through the required Terraform input, so an ordinary release never rebuilds or restarts the door. Same rule as codekeep, nethack, brogue, dopewars, and usurper. `dcss.yml` (PR/weekly build-validate of `docker/doors/dcss.Dockerfile`, publishes the pinned `door-dcss` image on main pushes, manual verify_deployed). License obligations tracked in `NOTICE` (GPL-2.0-or-later).
...
- **`macro_dir`/`save_dir` are compiled OUT of this build; never try to set either as an rc option or `-extra-opt-last`.** Root cause, confirmed against the pinned crawl 0.34.1 source: our Dockerfile builds `make install prefix=/opt/dcss`, and crawl's own `Makefile` has a rule (`ifneq ($(patsubst /opt%,%,$(prefix)),$(prefix))`) that force-sets `SAVEDIR := ~/.crawl` for any `/opt` prefix, which bakes `-DSAVE_DIR_PATH` into the binary. `initfile.cc` then registers `{"save_dir", "macro_dir"}` as `DisabledGameOption` whenever `SAVE_DIR_PATH` is defined, and `DisabledGameOption::loadFromParseState` (`game-options.cc`) returns an `"Option 'X' is disabled in this build."` error for **any** source, rc file or `-extra-opt-last` alike. **2026-08-05 incident:** a PR review flagged (incorrectly) that a pushed player rc could override `-macro` with its own `macro_dir =` line, since `-macro` only seeds `SysEnv.macro_dir` before the rc is parsed; the fix landed as a defensive `-extra-opt-last macro_dir=<dir>` re-force in `host.rs::crawl_args`, which broke every DCSS launch in prod the moment the new host image deployed (`Options error: Option 'macro_dir' is disabled in this build.`). Reverted same day. **The underlying premise was wrong**: because `macro_dir` is disabled outright on this build, a pushed rc's `macro_dir =` line is *also* rejected by the same `DisabledGameOption` path (as a normal, non-fatal parse-error in that context, unlike the fatal command-line form) — so there was never a cross-player hole to guard against. `-macro <dir>` alone (via `SysEnv.macro_dir`, consumed directly outside the option system) remains the correct and only mechanism. If a future crawl version or build recipe changes (e.g. `prefix` stops matching `/opt%`, or `SAVEDIR` gets explicitly overridden), re-verify against the actual Makefile before touching `crawl_args` again — reasoning from generic upstream docs without confirming our exact build flags is what caused this.
